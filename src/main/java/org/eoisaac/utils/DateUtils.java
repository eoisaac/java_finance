...
public class DateUtils { // Utility class for date formatting
  private static final DateTimeFormatter dateFormatter =
      DateTimeFormatter.ofPattern("dd/MM/yyyy"); // Sets the date format

  public static Instant convertStringToInstant(
      String dateString) { // Converts a string to an instant
  ...
  public static String convertInstantToString(Instant instant) { // Converts an instant to a string
    return convertInstantToString(
        instant,
        ZoneId.systemDefault()); // Returns the formatted string using the default system time zone
  }

  public static String convertInstantToString(Instant instant, ZoneId zoneId) {
    LocalDate localDate =
        instant.atZone(zoneId).toLocalDate(); // Convert the instant to the desired time zone
  ...
  private static Date convertLocalDateToDate(
      LocalDate localDate) { // Converts a local date to a date
    return Date.from(
        localDate.atStartOfDay(ZoneId.systemDefault()).toInstant()); // Returns the date
  }
}
