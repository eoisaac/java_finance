...
public class AppFrame extends JFrame {
  ...
  private boolean isFieldEmpty(
      String fieldValue, String errorMessage) { // Checks if the field is empty
  ...
  private boolean
      validateAndExtractFormFields() { // Validates and extracts the form fields to prevent errors
    ...
    if (isFieldEmpty(transactionName, "O nome da transação não pode ser vazio!")
        || isFieldEmpty(transactionValue, "O valor da transação não pode ser vazio!")
        || isFieldEmpty(entryDate, "A data da transação não pode ser vazia!")
        || isFieldEmpty(selectedCategory, "A categoria da transação não pode ser vazia!")) {
      return false;
    } // Check all fields to validate if they are empty

    String pattern =
        "\\d{2}/\\d{2}/\\d{4}"; // Sets the regex pattern to validate the date (dd/MM/yyyy)
  ...
  private void handleNewTransactionFormSubmit(ActionEvent evt) {
    if (!validateAndExtractFormFields()) { // Validates the form fields before creating the
                                           // transaction
      return;
    }
    ...
    TransactionType transactionType =
        isIncome
            ...
            : TransactionType.EXPENSE; // convert the boolean to TransactionType

    Instant entryDateInstant =
        DateUtils.convertStringToInstant(entryDate); // Converts the entry date to instant
    float transactionValueFloat =
        Float.parseFloat(transactionValue); // Converts the transaction value to float
    ...
    Optional<TransactionEntity> createdTransaction =
        transactionController.createTransaction(
            ...
            entryDateInstant,
            category.get()); // Creates the transaction
