...
public class AppFrame extends JFrame {
    ...
    private void createUIComponents() {
        ...
        transactionFormGroupLayout.setHorizontalGroup(transactionFormGroupLayout.createParallelGroup(GroupLayout.Alignment.LEADING).addGroup(transactionFormGroupLayout.createSequentialGroup().addContainerGap().addGroup(transactionFormGroupLayout.createParallelGroup(GroupLayout.Alignment.LEADING).addComponent(transactionNameField).addComponent(transactionCategoryComboBox).addComponent(transactionPriceField).addGroup(transactionFormGroupLayout.createSequentialGroup().addGroup(transactionFormGroupLayout.createParallelGroup(GroupLayout.Alignment.LEADING).addComponent(transactionNameLabel).addComponent(transactionCategoryLabel).addComponent(transactionPriceLabel).addComponent(transactionEntryDateLabel).addComponent(transactionEntryDateField, GroupLayout.PREFERRED_SIZE, 137, GroupLayout.PREFERRED_SIZE)).addGap(0, 0, Short.MAX_VALUE)).addGroup(GroupLayout.Alignment.TRAILING, transactionFormGroupLayout.createSequentialGroup().addComponent(transactionTypeIncomeRadioButton).addPreferredGap(LayoutStyle.ComponentPlacement.RELATED, GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE).addComponent(transactionTypeExpenseRadioButton, GroupLayout.PREFERRED_SIZE, 151, GroupLayout.PREFERRED_SIZE).addGap(4, 4, 4))).addContainerGap()).addComponent(separator).addGroup(GroupLayout.Alignment.TRAILING, transactionFormGroupLayout.createSequentialGroup().addContainerGap(81, Short.MAX_VALUE).addComponent(createTransactionButton, GroupLayout.PREFERRED_SIZE, 252, GroupLayout.PREFERRED_SIZE).addGap(69, 69, 69)));
        transactionFormGroupLayout.setVerticalGroup(transactionFormGroupLayout.createParallelGroup(GroupLayout.Alignment.LEADING).addGroup(transactionFormGroupLayout.createSequentialGroup().addContainerGap().addComponent(transactionNameLabel).addGap(6, 6, 6).addComponent(transactionNameField, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE, GroupLayout.PREFERRED_SIZE).addGap(18, 18, 18).addComponent(transactionCategoryLabel).addGap(6, 6, 6).addComponent(transactionCategoryComboBox, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE, GroupLayout.PREFERRED_SIZE).addGap(18, 18, 18).addComponent(transactionPriceLabel).addGap(6, 6, 6).addComponent(transactionPriceField, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE, GroupLayout.PREFERRED_SIZE).addGap(18, 18, 18).addComponent(transactionEntryDateLabel).addPreferredGap(LayoutStyle.ComponentPlacement.RELATED).addComponent(transactionEntryDateField, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE, GroupLayout.PREFERRED_SIZE).addGap(42, 42, 42).addComponent(separator, GroupLayout.PREFERRED_SIZE, 10, GroupLayout.PREFERRED_SIZE).addGap(28, 28, 28).addGroup(transactionFormGroupLayout.createParallelGroup(GroupLayout.Alignment.BASELINE).addComponent(transactionTypeIncomeRadioButton).addComponent(transactionTypeExpenseRadioButton)).addGap(39, 39, 39).addComponent(createTransactionButton).addContainerGap(73, Short.MAX_VALUE)));

        // Transactions Table
        transactionsTable.setBorder(BorderFactory.createEtchedBorder());
        transactionsTable.setFont(new Font("Segoe UI", Font.PLAIN, 14));
        transactionsTable.setModel(new DefaultTableModel(new Object[][]{}, new String[]{"ID", "Tipo", "Nome", "Classificação", "valor", "Data", "Cadastro"}));
        ...
        mainLayout.setHorizontalGroup(mainLayout.createParallelGroup(GroupLayout.Alignment.LEADING).addGroup(mainLayout.createSequentialGroup().addContainerGap().addGroup(mainLayout.createParallelGroup(GroupLayout.Alignment.LEADING, false).addComponent(transactionFormPanel, GroupLayout.DEFAULT_SIZE, GroupLayout.DEFAULT_SIZE, GroupLayout.PREFERRED_SIZE).addComponent(alertMessage, GroupLayout.DEFAULT_SIZE, GroupLayout.DEFAULT_SIZE, GroupLayout.PREFERRED_SIZE)).addPreferredGap(LayoutStyle.ComponentPlacement.RELATED).addGroup(mainLayout.createParallelGroup(GroupLayout.Alignment.LEADING, false).addGroup(mainLayout.createSequentialGroup().addComponent(totalIncomeLabel).addPreferredGap(LayoutStyle.ComponentPlacement.RELATED).addComponent(totalIncome).addGap(20, 20, 20).addComponent(totalExpenseLabel).addPreferredGap(LayoutStyle.ComponentPlacement.RELATED).addComponent(totalExpense).addPreferredGap(LayoutStyle.ComponentPlacement.RELATED, GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE).addComponent(totalBalanceLabel).addPreferredGap(LayoutStyle.ComponentPlacement.RELATED).addComponent(totalBalance).addGap(20, 20, 20).addComponent(deleteTransactionButton)).addComponent(transactionsTablePanel, GroupLayout.PREFERRED_SIZE, 600, GroupLayout.PREFERRED_SIZE))).addGroup(mainLayout.createSequentialGroup().addGap(363, 363, 363).addComponent(frameTitle).addContainerGap(16, Short.MAX_VALUE)));
        mainLayout.setVerticalGroup(mainLayout.createParallelGroup(GroupLayout.Alignment.LEADING).addGroup(GroupLayout.Alignment.TRAILING, mainLayout.createSequentialGroup().addComponent(frameTitle).addPreferredGap(LayoutStyle.ComponentPlacement.RELATED, 19, Short.MAX_VALUE).addGroup(mainLayout.createParallelGroup(GroupLayout.Alignment.LEADING, false).addComponent(transactionsTablePanel).addComponent(transactionFormPanel, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE, GroupLayout.PREFERRED_SIZE)).addPreferredGap(LayoutStyle.ComponentPlacement.RELATED).addGroup(mainLayout.createParallelGroup(GroupLayout.Alignment.BASELINE).addComponent(deleteTransactionButton).addComponent(totalIncomeLabel).addComponent(totalIncome).addComponent(totalExpenseLabel).addComponent(totalExpense).addComponent(totalBalanceLabel).addComponent(totalBalance).addComponent(alertMessage)).addContainerGap()));
    ...
    private void deleteTransaction(UUID transactionId) { // Deletes a transaction
        boolean deleted = transactionController.deleteTransaction(transactionId); // Deletes the transaction
    ...
    private void updateTransactionsData() { // Updates the transactions data on the table
        ...
        transactionsTable.setVisible(hasTransactions); // Shows the transactions table if there are transactions
    ...
    private boolean isFieldEmpty(String fieldValue, String errorMessage) { // Checks if the field is empty
        if (fieldValue.isEmpty()) { // If the field is empty
            alertMessage.setText(errorMessage); // Sets the error message based in the field
            return true; // Returns true
        }
        return false; // Returns false
    }

    private boolean validateAndExtractFormFields() { // Validates and extracts the form fields to prevent errors
        ...
        String selectedCategory = (String) transactionCategoryComboBox.getSelectedItem(); // Gets the selected category

        if (isFieldEmpty(transactionName, "O nome da transação não pode ser vazio!") || isFieldEmpty(transactionValue, "O valor da transação não pode ser vazio!") || isFieldEmpty(entryDate, "A data da transação não pode ser vazia!") || isFieldEmpty(selectedCategory, "A categoria da transação não pode ser vazia!")) {
            return false;
        } // Check all fields to validate if they are empty

        String pattern = "\\d{2}/\\d{2}/\\d{4}"; // Sets the regex pattern to validate the date (dd/MM/yyyy)
        if (!entryDate.matches(pattern)) { // If the date doesn't match the pattern
            alertMessage.setText("Selecione uma data válida!"); // Sets the error message
            return false; // Returns false
        }
    ...
    private void handleNewTransactionFormSubmit(ActionEvent evt) {
        if (!validateAndExtractFormFields()) { // Validates the form fields before creating the transaction
            return;
        }
        ...
        TransactionType transactionType = isIncome ? TransactionType.INCOME : TransactionType.EXPENSE; // convert the boolean to TransactionType

        Instant entryDateInstant = DateUtils.convertStringToInstant(entryDate); // Converts the entry date to instant
        float transactionValueFloat = Float.parseFloat(transactionValue); // Converts the transaction value to float

        Optional<CategoryEntity> category = categoryController.createCategory(selectedCategory); // Creates the category
        if (category.isEmpty()) { // Check if category was created in the database
            alertMessage.setText("Ocorreu um erro ao criar a categoria, tente novamente mais tarde!");
            return;
        }

        Optional<TransactionEntity> createdTransaction = transactionController.createTransaction(transactionName, transactionType, transactionValueFloat, entryDateInstant, category.get());// Creates the transaction

        if (createdTransaction.isEmpty()) { // Check if transaction was created in the database
            alertMessage.setText("Ocorreu um erro ao criar a transação, tente novamente mais tarde!");
            return;
        }

        transactions.add(createdTransaction.get()); // Updates the transactions list
        updateTransactionsData(); // Updates the transactions data on the table
        alertMessage.setText(""); // Clear the alert message
    ...
    public void handleSelectedTransaction(MouseEvent evt) { // Handles the selected transaction
        ...
        DefaultTableModel tableModel = (DefaultTableModel) transactionsTable.getModel(); // Gets the table model
        this.selectedTransactionId = (UUID) tableModel.getValueAt(selectedRow, 0); // Gets the selected transaction id

        deleteTransactionButton.setEnabled(true); // Enables the delete transaction button
    }

    private void handleDisplayConfirmDialog(ActionEvent actionEvent) { // Handles the display confirm dialog
        System.out.println("Display confirm dialog");
        int result = JOptionPane.showOptionDialog( // Displays the confirm dialog
    ...
    public void renderTransactionsSummary() { // Renders the transactions summary
        TransactionSummary transactionSummary = TransactionUtils.calculateTransactionSummary(transactions); // Calculates the transaction summary
        totalIncome.setText(CurrencyUtils.formatCurrency(transactionSummary.getTotalIncome())); // Sets the total income
        totalExpense.setText(CurrencyUtils.formatCurrency(transactionSummary.getTotalExpense())); // Sets the total expense
        totalBalance.setText(CurrencyUtils.formatCurrency(transactionSummary.getTotalBalance())); // Sets the total balance
    }
    ...
    private void renderTransactionsTable() { // Renders the transactions table
        ...
        transactionsTable.setDefaultRenderer(Object.class, new TransactionTableRowRenderer(hiddenColumnNames)); // Sets the table row to set the background color of the row (RED
        // or GREEN)
                ...
                transaction -> {
                    tableModel.addRow( // Adds the transaction to the table
                            new Object[]{ // Sets the transaction data
                                    ...
                                    DateUtils.convertInstantToString(transaction.getEntryDate()), // Sets the transaction entry date
                                    DateUtils.convertInstantToString(transaction.getCreatedAt()), // Sets the transaction created at date
                            });
                });
    }

    private void renderCategoriesComboBox() { // Renders the categories combo box
        DefaultComboBoxModel<String> comboBoxModel = (DefaultComboBoxModel<String>) transactionCategoryComboBox.getModel(); // Gets the combo box model
        comboBoxModel.removeAllElements(); // Removes all elements from the combo box
        categoryController.getAllCategories() // Gets all categories
